...
function xmlToPlainText(xml) {
  // Переносы абзацев -> \n, мягкие переносы строк и табы -> пробел,
  // затем убираем все теги. Без этого слова, разделённые Shift+Enter
  // внутри одного абзаца (частый случай в официальных шаблонах),
  // слипались бы в одно слово при поиске фраз.
  let text = xml
    .replace(/<w:p[ >]/gi, '\n<w:p ')
    .replace(/<w:br\s*\/?>/gi, ' ')
    .replace(/<w:tab\s*\/?>/gi, ' ')
...
// Сверка индекса УДК с темой статьи через Gemini (используем ту же модель,
// что и для ИИ-ассистента «Арай»). Если ключ API не задан, сеть недоступна
// или ответ модели неоднозначен — возвращаем null, и вызывающий код
// помечает пункт как «требует ручной проверки», а не как ошибку.
async function verifyUdkTopic(udkCode, topicHint) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey || !udkCode || !topicHint) return null;

  try {
    const prompt = `Ты — эксперт по библиотечной классификации УДК (Универсальная десятичная классификация), знаешь официальные таблицы УДК из открытых источников.
Индекс УДК в статье: "${udkCode}".
Тема статьи (по названию): "${topicHint}".
Определи по таблицам УДК, к какому разделу относится этот индекс, и соответствует ли он теме статьи.
Ответь СТРОГО в следующем формате, без лишнего текста:
Строка 1: одно слово — "ДА" (индекс соответствует теме) или "НЕТ" (индекс явно не соответствует теме).
Строка 2: краткое (одно предложение) объяснение, к какому разделу относится указанный индекс УДК.`;
    ...
    if (firstLine.includes('ДА')) return { ok: true, detail: text || `УДК ${udkCode} соответствует теме статьи.` };
    if (firstLine.includes('НЕТ')) return { ok: false, detail: text || `УДК ${udkCode} не соответствует теме статьи.` };
    return null;
  } catch (e) {
    console.error('Ошибка проверки УДК через Gemini:', e);
    return null;
  }
}
...
export async function checkDocx(buffer, meta = {}) {
  ...
  // Соответствие индекса УДК теме статьи (сверка по открытым таблицам УДК через ИИ)
  if (hasUDK) {
    const udkMatch = text.slice(0, 2000).match(/удк\s*[:\-–]?\s*([0-9][0-9.,:()\s–-]{0,30}[0-9)])/i);
    const udkCode = udkMatch ? udkMatch[1].trim() : null;
    const topicHint = meta.articleTitle || '';

    if (udkCode && topicHint) {
      const verdict = await verifyUdkTopic(udkCode, topicHint);
      if (verdict) {
        pass('Индекс УДК соответствует теме статьи', verdict.ok, verdict.detail);
      } else {
        manual('Соответствие индекса УДК теме статьи',
          `Индекс УДК: ${udkCode}. Не удалось автоматически сверить с открытыми таблицами УДК — проверьте вручную.`);
      }
    } else {
      manual('Соответствие индекса УДК теме статьи',
        'Не удалось распознать точный код УДК и/или тему статьи для автоматической сверки — проверьте вручную.');
    }
  }
  ...
  // Список литературы — количество источников (от 10 до 30, проверка нестрогая)
  ...
  const refsCountOk = refLines.length >= 10 && refLines.length <= 30;
  pass('Список литературы — от 10 до 30 источников', refsCountOk,
...
const APPLICATION_FIELDS = [
  { key: 'fullName', match: ['фамилия, имя, отчество', 'фио'], label: 'Фамилия, имя, отчество', required: true },
  { key: 'degree', match: ['учёная', 'ученая'], label: 'Учёная степень / звание', required: true },
  { key: 'position', match: ['должность или профессия', 'должность'], label: 'Должность или профессия', required: true },
  { key: 'organization', match: ['название организации'], label: 'Название организации (адрес)', required: true },
  { key: 'city', match: ['город, страна'], label: 'Город, страна', required: true },
  { key: 'email', match: ['адрес электронной почты', 'e-mail'], label: 'Email', required: true },
  { key: 'orcid', match: ['orcid'], label: 'ORCID', required: false },
  { key: 'phone', match: ['контактный телефон'], label: 'Контактный телефон (WhatsApp)', required: false },
  { key: 'articleTitle', match: ['название статьи'], label: 'Название статьи', required: true },
  { key: 'section', match: ['название секции'], label: 'Название секции конференции', required: true },
  { key: 'participationForm', match: ['форма участия'], label: 'Форма участия и выступления', required: true }
];

function extractTableRows(xml) {
  const tblMatch = xml.match(/<w:tbl\b[\s\S]*?<\/w:tbl>/i);
  if (!tblMatch) return null;
  const tbl = tblMatch[0];
  const rowMatches = tbl.match(/<w:tr\b[\s\S]*?<\/w:tr>/gi) || [];
  const rows = [];
  for (const row of rowMatches) {
    const cellMatches = row.match(/<w:tc\b[\s\S]*?<\/w:tc>/gi) || [];
    const cells = cellMatches.map(c => xmlToPlainText(c).replace(/\n/g, ' ').trim());
    if (cells.length) rows.push(cells);
  }
  return rows;
}

export async function checkApplicationDocx(buffer) {
  const results = [];
  const pass = (label, ok, detail) => results.push({ label, ok, detail, severity: 'auto' });
  const manual = (label, detail) => results.push({ label, ok: null, detail, severity: 'manual' });
  ...
  try {
    ...
    return {
      overallOk: false,
      results: [{ label: 'Формат файла заявки', ok: false, detail: 'Не удалось открыть файл как .docx (возможно, это .doc, .pdf или повреждённый файл)', severity: 'auto' }]
    };
  }

  pass('Файл заявки открывается корректно', true, 'Формат .docx распознан');

  const rows = extractTableRows(xml);

  if (!rows || rows.length < 3) {
    // Похоже, это не тот шаблон с таблицей — fallback на базовую проверку
    const text = xmlToPlainText(xml);
    const wordCount = countWords(text);
    pass('Заявка не пустая', wordCount >= 10, `Найдено слов: ${wordCount}`);
    manual(
      'Структура заявки (таблица с полями)',
      'Не удалось найти таблицу с полями по образцу официального шаблона — проверьте вручную, что заявка заполнена по форме.'
    );
    const overallOk = results.filter(r => r.severity === 'auto').every(r => r.ok);
    return { overallOk, results };
  }

  // Строим карту: первый столбец каждой строки -> второй столбец (значение)
  const rowMap = rows
    .filter(r => r.length >= 2)
    .map(r => ({ label: r[0].toLowerCase(), value: r[1] }));

  for (const field of APPLICATION_FIELDS) {
    const found = rowMap.find(r => field.match.some(m => r.label.includes(m)));
    if (!found) {
      if (field.required) {
        pass(`Поле «${field.label}»`, false, 'Не найдено в таблице заявки');
      } else {
        manual(`Поле «${field.label}»`, 'Не найдено в таблице — это поле необязательное, проверьте при необходимости.');
      }
      continue;
    }
    const filled = found.value && found.value.trim().length > 1;
    if (field.required) {
      pass(`Поле «${field.label}» заполнено`, filled, filled ? `«${found.value.trim()}»` : 'Поле пустое');
    } else if (filled) {
      pass(`Поле «${field.label}» заполнено`, true, `«${found.value.trim()}»`);
    }

    if (field.key === 'email' && filled) {
      const validEmail = /\S+@\S+\.\S+/.test(found.value);
      pass('Email в заявке выглядит корректным', validEmail, found.value.trim());
    }
  }

  // Проверку заголовка "ЗАЯВКА УЧАСТНИКА КОНФЕРЕНЦИИ" убрали намеренно —
  // люди оформляют шапку заявки по-разному, это не должно считаться ошибкой.
