...
export async function checkApplicationDocx(buffer) {
  ...
  const titleText = xmlToPlainText(xml).toLowerCase();
  pass('Заголовок «ЗАЯВКА УЧАСТНИКА КОНФЕРЕНЦИИ» присутствует',
    titleText.includes('заявка участника конференции'),
    titleText.includes('заявка участника конференции') ? 'Найден' : 'Не найден в начале документа');
